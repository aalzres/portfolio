...
private extension ColorPalette {
    var uiColor: UIColor {
        if let uiColor = UIColor(named: rawValue) {
            return uiColor
        }
        assertionFailure("This \(rawValue) color not implemented yet")
        return .white
    }
    
    func uiColor(_ tone: Tone) -> UIColor {
        if let uiColor = UIColor(named: rawValue + "_" + tone.rawValue) {
            return uiColor
        }
        assertionFailure("This \(rawValue) \(tone.rawValue) color not implemented yet")
        return .white
    }
}
